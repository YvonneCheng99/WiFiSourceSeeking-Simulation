...
random_distance = 0.4  # 刚开始随机方向移动每次移动的距离
v_track = 0.5 # 追踪方移动速度
random_times = 3  # 刚开始随机移动的次数
distance_1 = 0.3  # 向计算出的强度最大的位置移动的距离
distance_2 = 0.1
...
def track():
    ...
    while(1):
        if index < 3:
            ...
            df.to_csv('simulation_1.csv', mode='a', index=None, header=None)
            # 判断是否追踪到
            ...
            # 高斯过程
            max_cordinate_predicted, max_rssi_predicted = get_max_point(time_track)
            # 移动
            ...
            y = track_point[1]
            df = pd.DataFrame({"coordinate": [track_point], "t":time_track, "rssi": rssi})
            df.to_csv('simulation_4.csv', mode='a', index=None, header=None)
            # 判断是否追追踪到
            distance_x_this = abs(source_point[0] - track_point[0])
            distance_y_this = abs(source_point[1] - track_point[1])
            distance_this_2 = distance_x_this ** 2 + distance_y_this ** 2
