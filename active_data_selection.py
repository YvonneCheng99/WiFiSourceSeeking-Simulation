...
class ADS:
    ...
    def __init__(self, X, Y):
        self.dim = 3
        d1 = np.arange(-2.5, 2.5, 0.1)
        d2 = np.arange(-2.5, 2.5, 0.1)
        d1 = np.round(d1, 1)
        d2 = np.round(d2, 1)
    ...
    def calculate_candidates_points(self, current_position, candidates_angle_list, time_track, step_dis):
        ...
        for i in range(0, self.candidates_number):
            ...
            if 2.5 >= candidates_x >= -2.5 and 2.5 >= candidates_y >= -2.5:
                candidates_list.append([candidates_x, candidates_y, time_track])
        return candidates_list

    def next_step(self, calculate_times, current_position, Kff_inv, time_track, train_X, train_y, mu, cov, step_dis):
        ...
        '''
        index = self.likelihood_function(candidates_list, train_X, train_y, mu, cov, time_track)
        res = candidates_list[index]
        return res
        '''
        if calculate_times < self.times_threshold_value:
            index = self.ALC(train_X, candidates_list, Kff_inv, time_track)
            res = candidates_list[index]
            return res
        else:
            index = self.likelihood_function(candidates_list, train_X, train_y, mu, cov, time_track, Kff_inv)
            res = candidates_list[index]
            return res

    def get_reference_points_list(self,time_track):
        reference_d1 = np.arange(-2.5, 2.5, 1)
        reference_d2 = np.arange(-2.5, 2.5, 1)
